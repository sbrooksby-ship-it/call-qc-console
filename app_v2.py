...
def fetch_all_rows(table_name, order_col=None):
    ...
    default limit so growth over time never silently truncates the dashboard."""
...
if selected_tab == "📊 Performance Dashboard":
    ...
    try:
        ...
        if df.empty:
            ...
            # call_scores already comes in "long" format (one row per criteria
            # score), so no melt is needed — Raw Data -> Supabase already did
            # that flattening for us.

            # A single call is uniquely identified by its Date + Agent + Call
            # combination (this matches the unique constraint on the table),
            # so this plays the same role Unique_Row_ID played before.
            df['Unique_Row_ID'] = (
                df['Date'].astype(str) + '||' +
                df['Agent'].astype(str) + '||' +
                df['Call'].astype(str)
